...
def test_confidence_levels_explanation():
    ...
    if pairs_high:
        high_reasoning = " ".join(pairs_high[0].reasoning)
        assert "HIGH CONFIDENCE" in high_reasoning or pairs_high[0].correlation_strength >= 70
    ...
    if pairs_low:
        low_reasoning = " ".join(pairs_low[0].reasoning)
        # Should mention lower confidence or uncertainty
        assert pairs_low[0].correlation_strength < pairs_high[0].correlation_strength if pairs_high else True
        
        print("\nConfidence Level Explanations:")
        print(f"  High ({pairs_high[0].correlation_strength if pairs_high else 0:.1f}%): {pairs_high[0].reasoning[-1] if pairs_high else 'N/A'}")
        print(f"  Low ({pairs_low[0].correlation_strength if pairs_low else 0:.1f}%): {pairs_low[0].reasoning[-1] if pairs_low else 'N/A'}")
