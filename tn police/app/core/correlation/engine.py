...
class CorrelationEngine:
    ...
    def __init__(self, topology: Optional[TopologySnapshot] = None):
    ...
    def _create_session_pair(
        ...
        # Overall correlation strength (weighted average)
        time_weight = settings.WEIGHT_TIMING_SIMILARITY
        volume_weight = 0.3  # Hardcoded weight for volume
        pattern_weight = 0.3  # Hardcoded weight for pattern
        
        base_correlation = (
            time_score * time_weight +
            volume_score * volume_weight +
            (pattern_score or 0) * pattern_weight
        )
        
        # Explain composite scoring
        composite_explanation = f"Calculating composite correlation score using weighted average: Time ({time_weight*100:.0f}%) × {time_score:.1f}% = {time_score * time_weight:.1f}, Volume ({volume_weight*100:.0f}%) × {volume_score:.1f}% = {volume_score * volume_weight:.1f}"
        if pattern_score:
            composite_explanation += f", Pattern ({pattern_weight*100:.0f}%) × {pattern_score:.1f}% = {pattern_score * pattern_weight:.1f}"
        composite_explanation += f". Base correlation: {base_correlation:.1f}%."
